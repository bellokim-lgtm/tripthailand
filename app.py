...
# עיצוב CSS להתאמה לנייד ועיצוב טבלת ייאוש
st.markdown("""
    <style>
...
if days_left > 0:
    st.markdown(f"""
        <div class="countdown-box">
            <h2 style="margin:0; color:#d63384;">⏳ עוד {days_left} ימים למנייאק!</h2>
            <p style="text-align:center; direction:rtl; margin:0;">עד הטיול המטורף שלנו בפוקט וקאו לאק</p>
        </div>
    """, unsafe_allow_html=True)
elif days_left == 0:
    st.markdown("""<div class="countdown-box"><h2>✈️ טסים היום!!!</h2></div>""", unsafe_allow_html=True)
else:
    st.markdown("""<div class="countdown-box"><h2>🏝️ אנחנו בתאילנד! תהנו!</h2></div>""", unsafe_allow_html=True)
...
data = [
    ...
    {"תאריך": "23/05", "מקום": "מרכז באנג ניאנג", "פעילות": "זמן חוף, בריכה ושוק מקומי", "כתובת": "Khuekkhak, Takua Pa District, Phang-nga", "ניווט": "https://www.google.com/maps/search/?api=1&query=Bang+Niang+Market"},
    ...
    {"תאריך": "26/05", "מקום": "שמורת Khao Lak-Lam Ru", "פעילות": "טיול רגלי קל לחוף נסתר", "כתובת": "7/9 Moo 2, Thai Mueang, Phang-nga", "ניווט": "https://www.google.com/maps/search/?api=1&query=Khao+Lak+Lam+Ru+National+Park"},
    ...
    {"תאריך": "28/05", "מקום": "Khaolak Merlin Resort", "פעילות": "מעבר מלון (לילה 1) - חיות בגני המלון", "כתובת": "7/7 Moo 2, Lam Kaen, Thai Mueang, Phang Nga", "ניווט": "https://www.google.com/maps/search/?api=1&query=Khaolak+Merlin+Resort"},
    {"תאריך": "29/05", "מקום": "פארק המים Andamanda", "פעילות": "מפל Lampi בדרך ומעבר לפוקט", "כתובת": "333, Kathu, Kathu District, Phuket", "ניווט": "https://www.google.com/maps/search/?api=1&query=Andamanda+Phuket"},
    {"תאריך": "30/05", "מקום": "העיר העתיקה + טיסה", "פעילות": "Phuket Old Town וטיסה ב-22:30", "כתובת": "Vichitsongkram Rd, Wichit, Phuket", "ניווט": "https://www.google.com/maps/search/?api=1&query=Central+Phuket+Floresta"}
]

df = pd.DataFrame(data)

# --- גוף האפליקציה ---
st.title("🌴 האפליקציה שלכם לתאילנד")

mode = st.radio("תצוגה:", ["היום הנוכחי", "טיול מלא"], horizontal=True)

if mode == "היום הנוכחי":
    # ברירת מחדל לתאריך הנוכחי אם הוא בטווח, אחרת היום הראשון
    default_date = "20/05"
    selected_date = st.selectbox("בחר יום בטיול:", df["תאריך"].tolist())
    ...
    st.markdown("### 📋 כל המסלול מוכן מראש")
...
st.markdown("---")
st.markdown("<p style='text-align: center;'>🇹🇭 חופשה מהנה למשפחה הכי טובה בעולם! 🥥</p>", unsafe_allow_html=True)
